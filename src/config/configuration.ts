...
export default () => ({
  ...
  meta: {
    appName: process.env.APP_NAME ?? 'prepai',
    domainVerificationFile:
      process.env.DOMAIN_VERIFICATION_FILE ?? 'prepai-verify.txt',
  },
  rateLimit: {
    public: {
  ...
  security: {
    ...
    jwtSecret: process.env.JWT_SECRET ?? 'prepai',
  ...
  email: {
    name: process.env.EMAIL_NAME ?? 'prepai',
  ...
  tracking: {
    mode:
      ...
      'api-key',
    index: process.env.TRACKING_INDEX ?? 'prepai-logs',
